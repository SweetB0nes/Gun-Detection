...
class LiteProgressBar(pl.callbacks.ProgressBar):
    def __init__(self):
        super().__init__()
        self.last_epoch = 0
    ...
    def on_validation_batch_end(self, *args, **kwargs):
        ...
        trainer = args[0]
        self.timer.update(float(trainer.val_batch_idx)/float(trainer.total_val_batches))
        log = copy.deepcopy(trainer.logged_metrics)
        if 'epoch' in log:
            log['Info/epoch'] = copy.deepcopy(log['epoch'])
            del log['epoch']
        log['Info/Mode'] = 'val'
        log['Info/Progress'] = progress_str(15, float(trainer.val_batch_idx)/float(trainer.total_val_batches)) + ' ' + str(trainer.val_batch_idx) + ' / ' + str(trainer.total_val_batches)
        log['Info/Time'] = str(self.timer)
        self.pbar.update(log)